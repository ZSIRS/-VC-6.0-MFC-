...
void CTeacherDlg::OnButtonAdd() 
{
	...
	{
        AfxMessageBox("插入的记录中字段值不能为空");
        return;
	}
	try
	{
		m_pRecordSet->AddNew();
		m_pRecordSet->PutCollect("班级",_variant_t(m_class));
		m_pRecordSet->PutCollect("学号",_variant_t(m_num));
		m_pRecordSet->PutCollect("性别",_variant_t(m_sex));
	    m_pRecordSet->PutCollect("姓名",_variant_t(m_name));
		m_pRecordSet->PutCollect("金工实习理论",_variant_t(m_one));
		m_pRecordSet->PutCollect("数字电子技术基础",_variant_t(m_two));
		m_pRecordSet->PutCollect("电路",_variant_t(m_three));
		m_pRecordSet->PutCollect("电路实验",_variant_t(m_four));
		m_pRecordSet->PutCollect("数字电子技术基础实验",_variant_t(m_five));
		m_pRecordSet->PutCollect("概率统计B",_variant_t(m_six));
		m_pRecordSet->PutCollect("复变函数与积分变换",_variant_t(m_seven));
		m_pRecordSet->PutCollect("大学英语3",_variant_t(m_eight));
		m_pRecordSet->PutCollect("毛概",_variant_t(m_nine));
        m_pRecordSet->PutCollect("体育3",_variant_t(m_ten));
		m_pRecordSet->PutCollect("职业核心能力1",_variant_t(m_eleven));
		m_pRecordSet->Update();
